...
-- Corridor tables (populated by openinterstate-import --build-corridors-only after this SQL)
...
BEGIN
    IF to_regclass('public.osm2pgsql_v2_exits_nodes') IS NULL
       ...
       OR to_regclass('public.osm2pgsql_v2_highways') IS NULL THEN
        RAISE EXCEPTION
            'Missing canonical osm2pgsql tables. Run pipeline/bin/import_canonical_osm.sh first.';
    END IF;
END $$;
...
-- by openinterstate-import steps that run after this SQL in the pipeline:
